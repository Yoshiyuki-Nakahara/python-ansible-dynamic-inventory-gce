...
class GceInventory(object):
    ...
    def group_instances(self, zones=None):
        ...
        for node in self.list_nodes():
            ...
            if self.config.getboolean('inventory', 'inventory_group_zone'):
            ...
            if self.config.getboolean('inventory', 'inventory_group_instance_tags'):
                tags = node.extra['tags']
                for t in tags:
                    if t.startswith('group-'):
                        tag = t[6:]
                    else:
                        tag = self.config.get('inventory', 'inventory_group_instance_tags_prefix') + '%s' % t
                    if tag in groups:
                        groups[tag]['hosts'].append(name)
                    else:
                        groups[tag] = dict()
                        groups[tag]['hosts'] = [name]

            if self.config.getboolean('inventory', 'inventory_group_zone'):
                net = node.extra['networkInterfaces'][0]['network'].split('/')[-1]
                net = self.config.get('inventory', 'inventory_group_running_status_prefix') + '%s' % net
                if net in groups:
                    groups[net].append(name)
                else:
                    groups[net] = [name]

            if self.config.getboolean('inventory', 'inventory_group_machine_type'):
            ...
            if self.config.getboolean('inventory', 'inventory_group_image'):
            ...
            if self.config.getboolean('inventory', 'inventory_group_running_status'):
                status = node.extra['status']
                stat = self.config.get('inventory', 'inventory_group_running_status_prefix') + '%s' % status.lower()
                if stat in groups:
